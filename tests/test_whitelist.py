...
class TestStringMethods(unittest.TestCase):
    ...
    def test_ScanResults_can_write_to_disk_with_no_results(self):
        ...
        assert len({secret for secret in scan.reconciled_results if secret.acknowledged == True}) == 0
        assert len({secret for secret in scan.reconciled_results if secret.acknowledged == False}) == 2
    ...
    def test_ScanResults_can_write_to_disk_with_some_acknowledged_results(self):
        ...
        assert len({secret for secret in scan.reconciled_results if secret.acknowledged == True}) == 1
        assert len({secret for secret in scan.reconciled_results if secret.acknowledged == False}) == 1
